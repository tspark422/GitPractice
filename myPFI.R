myPFI <- function(dat, M=100, max.iter=50, eps=1e-09){
  ...
  for(iter in 1:max.iter){
    ...
    information <- matrix(c(U11, U12, U12, U22), nrow=2)
    beta_new = beta_old - solve(information)%*%c(score1, score2)
    lm(y1_mis~x, data=idat, weights=weight)
    ...
    for(l in 1:max.iter){
      ...
      information = matrix(c(U_logis_11, U_logis_12, U_logis_13, U_logis_12, U_logis_22, U_logis_23, U_logis_13, U_logis_23, U_logis_33), nrow=3)
      pi_new = phi_old_NR + solve(information)%*%c(score_logis_1, score_logis_2, score_logis_3)
      eps_logis = norm(pi_new-phi_old_NR, type='2')
      phi_old_NR=pi_new
    }
    eps=norm(c(beta_new-beta_old, sigma2_new-sigma2_old, pi_new-phi_old), type='2')
    beta_old=beta_new; sigma2_old=sigma2_new; phi_old=pi_new
  }
}
