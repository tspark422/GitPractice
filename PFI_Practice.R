myPFI <- function(n, beta, sigma, phi, M=100, max.iter=50, eps=1e-09, B=2000){
  ...
  rst.beta <- matrix(0, nrow=B, ncol=2)
  rst.sigma <- rep(0, B)
  rst.phi <- matrix(0, nrow=B, ncol=3)
  ...
  rst.beta[1, ] = beta_new
  rst.sigma[1] = sigma2_new
  rst.phi[1, ] = phi_new
  rst.eta1[1, ] = c(eta1, est_impvar1, est_compvar1)
  rst.eta2[1, ] = c(eta2, est_impvar2, est_compvar2)
  rst.eta4[1, ] = round(c(eta4, est_impvar4, est_compvar4), 5)
  
  return(list(beta=rst.beta[1, ], sigma=rst.sigma[1], phi=rst.phi[1, ], eta1=rst.eta1[1, ], eta2=rst.eta2[1, ], eta4=rst.eta4[1, ]))
  ...
  #   print(apply(rst.pi[1:a,],2,mean))   
  #   print(apply(rst.pi[1:a,],2,sd))  
